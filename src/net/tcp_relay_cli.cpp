...
constexpr std::string_view kClientProfileUriPrefix{"fps://v1/"};
...
[[nodiscard]] auto encode_client_profile_uri(std::string_view json_text) -> std::string {
    return std::string{kClientProfileUriPrefix} + base64url_encode(json_text);
}
...
[[nodiscard]] auto decode_client_profile_uri(std::string_view uri) -> Result<std::string, std::string> {
    ...
    auto parsed = json::parse(decoded.value(), error);
    if(error) {
    ...
    return Result<std::string, std::string>::success(json::serialize(parsed.as_object()));
}
