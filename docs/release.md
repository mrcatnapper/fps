...
This checklist is for private/public beta release candidates. It does not
publish artifacts by itself.

The project license is MIT; keep `LICENSE` in every source distribution.

This checklist remains focused on beta release candidates. Private image
publication, signed artifacts and public release uploads are deliberately
separate manual steps until release policy is finalized.
...
The private repository has one manual image workflow:

- `Publish Private Images` performs the Docker runtime smoke and can push
  Ubuntu and Alpine images to GHCR when `publish=true`.
...
Recommended private GHCR tags for a pre-release:
...
or document `latest` until public release policy is explicit. The private beta
publish workflow intentionally disables Buildx provenance/SBOM attestations so
GHCR shows only the two runtime image tags above; signed images and provenance
remain explicit future release-hardening work.
