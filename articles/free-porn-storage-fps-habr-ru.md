...
дополнительные аутентифицируемые данные AEAD, кэш повторов.

Есть зафиксированная проблема для следующих итераций исправлений: текущий
Zero-RTT-кандидат (сообщение рукопожатия от клиента серверу)
начинается с 32-байтного значения, похожего на открытый ключ, в
фиксированной позиции. Оно задумано как сеансовое и одноразовое,
но видимая структура повторяющегося блока сама
по себе является более простой целью для классификатора, чем тайминговый анализ.
Следующая версия должна заменить это другим механизмом, который так же позволит серверу быстро опознать вероятного
клиента, не оставляя стабильного открытого идентификатора в начале полезной
нагрузки.
...
исправление видимого Zero-RTT-префикса, проектирование Android-клиента через
