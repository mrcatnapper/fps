...
- [UPDATE: записи FPS теперь отдельные TLS-записи](#update-separate-tls-records)
- [UPDATE: эксперимент с pcap-захватом](#update-pcap-experiment)
- [Ссылки](#references)
...
так сказать, удачи с поиском. FPS — это экспериментальный фреймворк для передачи скрытых
датаграмм внутри живых TLS-сессий; Linux TUN VPN — первый и самый важный продуктовый
адаптер поверх этого ядра.
...
успешно, обе стороны начинают распознавать отдельные классифицируемые записи FPS:
на проводе всё ещё видны записи TLS Application Data, но их полезная нагрузка
содержит зашифрованные, понятные только клиенту и серверу FPS, датаграммы,
управляющие сообщения и паддинг.
...
Архитектурно FPS теперь состоит из переиспользуемого ядра скрытого датаграммного
транспорта и первого продуктового адаптера под Linux TUN. Ядро регистрирует
аутентифицированные сессии-носители, фрагментирует и собирает best-effort
непрозрачные датаграммы и планирует их поверх одного или нескольких носителей.
Linux-адаптер создаёт TUN-интерфейсы на обеих сторонах: приложения отправляют
IP-пакеты в TUN, а адаптер отображает эти пакеты в непрозрачные датаграммы FPS.
...
прозрачный ретранслятор. После переключения обычные TLS-записи носителя
продолжают пересылаться побайтово как есть, а FPS вставляет между ними отдельные
классифицируемые TLS Application Data records с зашифрованными датаграммами и
...
код разделён так, чтобы аутентификация, классификация записей, пул носителей и
фрейминг непрозрачных датаграмм могли переиспользоваться за Android `VpnService`
или другим будущим адаптером, не наследуя Linux TUN, IPv4 lease-политику и вызовы
`ip`.
...
Любая успешно аутентифицированная TLS-сессия может переносить непрозрачные
датаграммы FPS. В FPS нет понятия единственной главной сессии, поэтому несколько
вкладок браузера или прикладных потоков могут стать общим пулом носителей. В
текущем VPN-адаптере эти датаграммы являются TUN-пакетами; в будущем тот же core
может обслуживать другой ненадёжный payload без изменения слоя аутентификации
носителей.
...
Текущий продуктовый адаптер FPS переносит IP-пакеты через Linux TUN. Поверх него
можно поднять SOCKS для удобства — в документации есть пример с SOCKS-прокси
Dante как надстройкой, — но скрытый транспорт под ним является обобщённым
best-effort каналом непрозрачных датаграмм. TUN остаётся первым и самым важным
адаптером, потому что превращает FPS в VPN-фреймворк, но это уже не жёсткая
граница самого протокола.

<a id="shaping"></a>

### Пространство для шейпинга трафика

В бете уже есть фрагментация непрозрачных датаграмм и точки расширения для
паддинга внутри конвертов. Для текущего TUN-адаптера это означает, что
около-MTU IP-пакеты можно резать на меньшие FPS-записи. Будущий слой формирования
трафика сможет решать, сколько скрытых байт добавлять в носитель, как резать
payload, как ограничивать скрытый трафик и как распределять нагрузку между
сессиями-носителями.
...
поздно переключает уже существующий поток и переносит не прокси-запросы, а
непрозрачные датаграммы через пул сессий-носителей. В текущем Linux VPN-адаптере
эти датаграммы соответствуют TUN-пакетам.
...
<a id="update-separate-tls-records"></a>

## UPDATE: записи FPS теперь отдельные TLS-записи

После первой версии черновика реализация перешла к более чистой модели в
проводе. После поздней аутентификации FPS больше не обязан заменять каждую
запись TLS-носителя одним большим зашифрованным конвертом, внутри которого
перемешаны байты носителя и скрытая полезная нагрузка. Обычные TLS-записи
носителя продолжают пересылаться побайтово как есть, а FPS вставляет между ними
собственные отдельные классифицируемые записи TLS Application Data.

Практически это значит следующее: на участке `fps_client <-> fps_server`
видимая запись FPS всё ещё выглядит как обычная TLS Application Data record, но
внутри несёт зашифрованный пакет FPS с непрозрачными датаграммами, управляющими
сообщениями и паддингом. Текущий Linux TUN-адаптер отображает один VPN-пакет в
одну такую датаграмму до внутренней фрагментации. Реальные TLS-записи
браузера/сервера остаются в том же упорядоченном TCP-потоке и не терминируются
FPS.

Такая форма протокола удобнее для будущего шейпинга и троттлинга. Теперь
скрытый канал состоит из явных единиц планирования: FPS может решать, когда
вставлять свои TLS-похожие записи, какого они должны быть размера, сколько
паддинга добавить и насколько агрессивно подстраиваться под наблюдаемую
плотность трафика носителя. Иными словами, корректность и конфиденциальность
уже не требуют преобразовывать каждую TLS-запись носителя; слой формирования
трафика сможет работать с дополнительными TLS-образными записями.

Ограничение при этом никуда не исчезло: FPS синтаксически выглядит как TLS, но
его ещё нужно статистически приводить к форме выбранного носителя. Отдельные
TLS-записи делают эту работу проще, но не выполняют её автоматически.

<a id="update-pcap-experiment"></a>

## UPDATE: эксперимент с pcap-захватом

Чтобы не спорить о форме трафика только на уровне интуиции, я отдельно снял
pcap с видимого TCP-потока `fps_client <-> fps_server`. Эксперимент использовал
локальную Docker/TUN-топологию: один `fps_client`, один `fps_server`,
отладочный HTTPS/WSS-носитель и двунаправленный UDP `iperf3` поверх выданного
FPS TUN-адреса. Zero-RTT-переключение было намеренно задержано, чтобы в захвате
осталось небольшое окно "до" и более длинное окно "после".

Полный отчёт лежит в репозитории:
[`docs/pcap-flow-analysis.md`](../docs/pcap-flow-analysis.md). Короткий вывод:
Wireshark/libpcap по-прежнему видят на FPS-соединении корректно разбираемые
TLS-записи, но распределения размеров пакетов и интервалов между пакетами
заметно меняются после того, как FPS начинает переносить TUN-трафик.

![Обзор размеров пакетов и межпакетных интервалов на видимом FPS TCP-соединении](../docs/assets/pcap-flow/bidir-overview.png)

![Квантили размеров пакетов и межпакетных интервалов по направлениям](../docs/assets/pcap-flow/bidir-quantiles.png)

В конкретном прогоне двунаправленный UDP-тест прошёл примерно на 2 Мбит/с в
каждую сторону без потерь. До переключения окно носителя было небольшим:
медианный размер IP-пакета около 119 байт, p95 межпакетного интервала около
33 мс. После переключения видимый FPS-линк нёс около 8.8 Мбит/с IP-трафика,
медианный размер пакета вырос примерно до 1321 байта, а p95 межпакетного
интервала снизился примерно до 4.3 мс.

Вывод не в том, что FPS уже неотличим от носителя. Наоборот, полезный вывод
как раз в другом: текущая бета сохраняет синтаксис TLS-записей, но
дополнительная нагрузка видна как плотная near-MTU-полоса после переключения.
Следующая серьёзная работа по анализу трафика должна быть посвящена не только
валидности TLS-записей, а планированию, ограничению скорости, паддингу и
выравниванию классифицируемых FPS-записей под конкретный профиль носителя.
