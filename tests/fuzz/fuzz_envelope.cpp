...
auto frame_type(std::uint8_t value) -> fps::FrameType {
    constexpr std::array values{
        fps::FrameType::opaque_datagram,          fps::FrameType::ping,    fps::FrameType::pong, fps::FrameType::flow_control, fps::FrameType::close,
        fps::FrameType::opaque_datagram_fragment, fps::FrameType::control,
    };
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    ...
    for(std::size_t i = 0; i < frame_count; ++i) {
        ...
        payload.assign(input.begin() + static_cast<std::ptrdiff_t>(offset), input.begin() + static_cast<std::ptrdiff_t>(offset + payload_size));
        content.frames.push_back(
