...
auto binding(std::span<const std::byte> input) -> fps::ZeroRttChannelBinding {
  ...
  for (std::size_t i = 0; i < out.previous_record_hash.size(); ++i) {
    out.previous_record_hash[i] =
        i < input.size() ? input[i] : static_cast<std::byte>(i);
  }
  return out;
}
...
auto server_config(const fps::X25519KeyPair& server, const fps::X25519KeyPair& client)
    -> fps::ZeroRttUpgradeConfig {
  return fps::ZeroRttUpgradeConfig{
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
  ...
  (void)server_engine.verify_client_upgrade(input, 1000, channel);
  ...
  auto built = client_engine.build_client_upgrade(1000,
                                                  channel,
                                                  padding,
                                                  key_pair(0xa0),
                                                  nonce(0x44));
  if (built) {
    ...
    (void)roundtrip_server.verify_client_upgrade(built.value().wire, 1001, channel);
  }
