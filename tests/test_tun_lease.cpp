...
struct TempDir {
    ...
    TempDir() {
        path = std::filesystem::temp_directory_path() / ("fps-lease-test-" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
...
BOOST_AUTO_TEST_CASE(lease_allocator_lists_removes_and_prunes_leases) {
    ...
    BOOST_TEST(reloaded_entries.value()[0].client_public_key_base64 == fps::base64_encode(first_key));
}
