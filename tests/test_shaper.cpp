...
auto payload(std::size_t size) -> fps::ByteVector {
  return fps::ByteVector(size, std::byte{0x5a});
}
...
BOOST_AUTO_TEST_CASE(enforces_cover_ratio_budget) {
  ...
  BOOST_TEST(shaper.covert_bytes_planned(fps::Direction::client_to_server) ==
             plan.covert_payload_budget);
}
...
BOOST_AUTO_TEST_CASE(direction_state_is_independent) {
  fps::Shaper shaper{sample_profile()};
  const auto client_data = payload(32);
  const auto server_data = payload(32);

  shaper.observe_cover_record({fps::Direction::client_to_server, 1000, {}});
  shaper.enqueue_covert_payload(
      {fps::Direction::client_to_server, client_data, fps::Priority::normal});
  shaper.enqueue_covert_payload(
      {fps::Direction::server_to_client, server_data, fps::Priority::normal});
