...
constexpr std::size_t kCovertPlainHeaderSize = 1U + 1U + sizeof(std::uint32_t) +
                                                sizeof(std::uint32_t);
...
auto expected_tls_record_size(std::size_t payload_size) -> std::size_t {
  return kTlsHeaderSize + kCovertWireSequenceSize + kCovertPlainHeaderSize +
         payload_size + kAeadTagSize;
}

auto session_keys() -> fps::SessionKeys {
  fps::SessionKeys keys;
  for (std::size_t i = 0; i < keys.client_to_server.key.size(); ++i) {
    keys.client_to_server.key[i] = static_cast<std::byte>(0x31U + i);
    keys.server_to_client.key[i] = static_cast<std::byte>(0x91U + i);
  }
  for (std::size_t i = 0; i < keys.client_to_server.nonce_salt.size(); ++i) {
    keys.client_to_server.nonce_salt[i] = static_cast<std::byte>(0x41U + i);
    keys.server_to_client.nonce_salt[i] = static_cast<std::byte>(0xa1U + i);
  }
  return keys;
}
...
auto read_peer_packet(boost::asio::io_context& io,
                      boost::asio::posix::stream_descriptor& peer,
...
BOOST_AUTO_TEST_CASE(write_packet_rejects_invalid_packets_and_full_queue) {
  ...
  auto pump = fps::net::TunPacketPump::create(
      ...
      fds[0].release(),
      manager,
      fps::net::TunPacketPumpConfig{.mtu = 4, .max_write_queue_packets = 1});
...
BOOST_AUTO_TEST_CASE(read_packet_from_fd_enqueues_tun_frame_to_carrier_session) {
  ...
  auto pump = fps::net::TunPacketPump::create(fixture.io,
                                              fds[0].release(),
                                              manager,
                                              fps::net::TunPacketPumpConfig{.mtu = 64});
...
BOOST_AUTO_TEST_CASE(read_packet_without_carrier_reports_session_error_and_continues) {
  ...
  auto pump = fps::net::TunPacketPump::create(
      ...
      fps::net::TunPacketPumpHandlers{
          .on_session_error =
              [&](fps::net::SessionManagerError error) {
                session_errors.push_back(error);
              },
          .on_error = {},
          .on_read_packet = {},
          .on_write_packet = {},
...
BOOST_AUTO_TEST_CASE(session_manager_sink_can_write_inbound_packet_to_fd) {
  ...
  pump = fps::net::TunPacketPump::create(io,
                                         fds[0].release(),
                                         manager,
                                         fps::net::TunPacketPumpConfig{.mtu = 64});
  boost::asio::posix::stream_descriptor peer{io, fds[1].release()};
