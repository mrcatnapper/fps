...
auto payload(std::initializer_list<unsigned int> values) -> fps::ByteVector {
  fps::ByteVector out;
...
auto keys() -> fps::SessionKeys {
  fps::SessionKeys session_keys;
  for (std::size_t i = 0; i < session_keys.client_to_server.key.size(); ++i) {
    session_keys.client_to_server.key[i] = static_cast<std::byte>(0x10U + i);
    session_keys.server_to_client.key[i] = static_cast<std::byte>(0x80U + i);
  }
  for (std::size_t i = 0; i < session_keys.client_to_server.nonce_salt.size(); ++i) {
    session_keys.client_to_server.nonce_salt[i] = static_cast<std::byte>(0xa0U + i);
    session_keys.server_to_client.nonce_salt[i] = static_cast<std::byte>(0xb0U + i);
  }
  return session_keys;
}
...
BOOST_AUTO_TEST_CASE(aead_roundtrip_for_all_frame_types) {
  ...
  for (const auto frame_type : frame_types) {
    const auto data = payload({1, 2, 3, static_cast<unsigned int>(frame_type)});
...
BOOST_AUTO_TEST_CASE(tampered_ciphertext_or_tag_fails) {
  fps::CovertCodec client{client_config()};
  fps::CovertCodec server{server_config()};
  const auto data = payload({0xaa, 0xbb, 0xcc});
...
BOOST_AUTO_TEST_CASE(padding_is_stripped_and_reported) {
  fps::CovertCodec client{client_config()};
  fps::CovertCodec server{server_config()};
  const auto data = payload({0x11, 0x22});
