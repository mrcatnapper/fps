...
auto repeated(std::byte value, std::size_t size) -> fps::ByteVector {
  return fps::ByteVector(size, value);
}
...
BOOST_AUTO_TEST_CASE(aead_rejects_invalid_nonce_or_tag_size) {
  ...
  auto decrypted = fps::aead_chacha20_poly1305_decrypt(key(),
                                                       valid_nonce,
                                                       aad,
                                                       valid.value().ciphertext,
                                                       short_tag);
