...
auto session_keys() -> fps::SessionKeys {
  fps::SessionKeys keys;
  for (std::size_t i = 0; i < keys.client_to_server.key.size(); ++i) {
    keys.client_to_server.key[i] = static_cast<std::byte>(0x61U + i);
    keys.server_to_client.key[i] = static_cast<std::byte>(0xb1U + i);
  }
  for (std::size_t i = 0; i < keys.client_to_server.nonce_salt.size(); ++i) {
    keys.client_to_server.nonce_salt[i] = static_cast<std::byte>(0x71U + i);
    keys.server_to_client.nonce_salt[i] = static_cast<std::byte>(0xc1U + i);
  }
  return keys;
}
...
auto pipeline(fps::Direction send_direction) -> fps::CoverSessionPipeline {
  return fps::CoverSessionPipeline{fps::CovertCodec{codec_config(send_direction)}};
}
...
void run_until(boost::asio::io_context& io, Predicate predicate) {
  for (auto i = 0; i < 200 && !predicate(); ++i) {
    io.run_for(std::chrono::milliseconds{5});
    io.restart();
  }
}

auto read_queued_bytes(boost::asio::io_context& io,
                       tcp::socket& socket,
                       std::size_t size) -> fps::ByteVector {
...
auto decode_tun_record(fps::Direction wire_direction, const fps::ByteVector& received)
    -> fps::DecodedFrame {
...
auto decode_frames(fps::Direction wire_direction, const fps::ByteVector& received)
    -> std::vector<fps::DecodedFrame> {
  auto receiver = pipeline(fps::opposite_direction(wire_direction));
  const auto result = receiver.process_inbound_tls(received);
...
auto fragment_payload(std::uint32_t packet_id,
                      std::uint16_t fragment_index,
                      std::uint16_t fragment_count,
                      std::uint32_t total_size,
                      const fps::ByteVector& chunk) -> fps::ByteVector {
  fps::ByteVector out;
...
struct CodecSessionFixture {
  ...
  explicit CodecSessionFixture(std::size_t max_write_queue_bytes = 1024U * 1024U)
...
BOOST_AUTO_TEST_CASE(carrier_pool_round_robins_tun_packets_across_sessions) {
  ...
  auto first_received =
      read_queued_bytes(first.io, first.origin_pair.external, first_queued.value());
  auto second_received =
      read_queued_bytes(second.io, second.origin_pair.external, second_queued.value());
...
BOOST_AUTO_TEST_CASE(server_lease_routing_round_robins_same_client_carriers) {
  ...
  BOOST_REQUIRE(second_queued);
  auto first_received =
      read_queued_bytes(first.io, first.client_pair.external, first_queued.value());
  auto second_received =
      read_queued_bytes(second.io, second.client_pair.external, second_queued.value());
  BOOST_CHECK(decode_tun_record(fps::Direction::server_to_client, first_received)
                  .payload == ipv4_packet(server_ip, client_ip));
  BOOST_CHECK(decode_tun_record(fps::Direction::server_to_client, second_received)
                  .payload == ipv4_packet(server_ip, client_ip));
}
...
BOOST_AUTO_TEST_CASE(server_allows_multiple_carriers_for_same_client_instance) {
  ...
  auto first_registered =
      manager.add_carrier_session_with_metadata(first.session, client_ip, instance);
  auto second_registered =
      manager.add_carrier_session_with_metadata(second.session, client_ip, instance);
...
BOOST_AUTO_TEST_CASE(server_replaces_old_carriers_for_different_client_instance) {
  ...
  fps::net::SessionManager manager{
      ...
      fps::net::SessionManagerHandlers{
          ...
          .on_event =
              [&](fps::net::SessionManagerEvent event) {
                events.push_back(event);
              },
      }};
  BOOST_CHECK(
      manager.add_carrier_session_with_metadata(old_first.session, client_ip, old_instance)
          .added);
  BOOST_CHECK(
      manager.add_carrier_session_with_metadata(old_second.session, client_ip, old_instance)
          .added);

  auto registered =
      manager.add_carrier_session_with_metadata(replacement.session, client_ip, new_instance);
  ...
  BOOST_REQUIRE(queued);
  auto received =
      read_queued_bytes(replacement.io, replacement.client_pair.external, queued.value());
  BOOST_CHECK(decode_tun_record(fps::Direction::server_to_client, received).payload ==
              outbound);
  ...
  manager.handle_covert_frame(
      replacement.session, fps::Direction::client_to_server, replacement_frame);
...
BOOST_AUTO_TEST_CASE(carrier_pool_skips_full_carrier_for_next_packet) {
  ...
  BOOST_REQUIRE(queued);
  auto received =
      read_queued_bytes(available.io, available.origin_pair.external, queued.value());
  auto frame = decode_tun_record(fps::Direction::client_to_server, received);
  BOOST_CHECK(frame.payload == packet);
      ...
      fps::net::SessionManagerHandlers{
          .on_tun_packet =
              [&](fps::ByteVector reassembled) {
                packets.push_back(std::move(reassembled));
              },
          .on_event =
              [&](fps::net::SessionManagerEvent event) {
                events.push_back(event);
...
BOOST_AUTO_TEST_CASE(server_source_enforcement_drops_missing_lease) {
  ...
  manager.handle_covert_frame(carrier_without_lease.session,
                              fps::Direction::client_to_server,
                              frame);
...
BOOST_AUTO_TEST_CASE(fragmented_inbound_packet_is_reassembled_before_source_enforcement) {
  ...
  fps::DecodedFrame first;
  first.frame_type = fps::FrameType::tun_packet_fragment;
  first.payload = fragment_payload(42, 0, 2, static_cast<std::uint32_t>(packet.size()),
                                   fps::ByteVector{packet.begin(), packet.begin() + 10});
  fps::DecodedFrame second;
  second.frame_type = fps::FrameType::tun_packet_fragment;
  second.payload = fragment_payload(42, 1, 2, static_cast<std::uint32_t>(packet.size()),
                                    fps::ByteVector{packet.begin() + 10, packet.end()});
