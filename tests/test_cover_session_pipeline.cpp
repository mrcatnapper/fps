...
auto session_keys() -> fps::SessionKeys {
  ...
  for (std::size_t i = 0; i < keys.client_to_server.key.size(); ++i) {
    keys.client_to_server.key[i] = static_cast<std::byte>(0x21U + i);
    keys.server_to_client.key[i] = static_cast<std::byte>(0xa1U + i);
  }
  for (std::size_t i = 0; i < keys.client_to_server.nonce_salt.size(); ++i) {
    keys.client_to_server.nonce_salt[i] = static_cast<std::byte>(0x31U + i);
    keys.server_to_client.nonce_salt[i] = static_cast<std::byte>(0xb1U + i);
  }
  return keys;
}
...
auto client_pipeline() -> fps::CoverSessionPipeline {
  return fps::CoverSessionPipeline{
      fps::CovertCodec{codec_config(fps::Direction::client_to_server)}};
}

auto server_pipeline() -> fps::CoverSessionPipeline {
  return fps::CoverSessionPipeline{
      fps::CovertCodec{codec_config(fps::Direction::server_to_client)}};
}
...
BOOST_AUTO_TEST_CASE(passthrough_forwards_application_data_even_when_sequence_like) {
  ...
  const auto real_record = tls_app_record({0x00,
                                           0x00,
                                           0x00,
                                           0x00,
                                           0x00,
...
BOOST_AUTO_TEST_CASE(buffers_partial_tls_record_until_complete) {
  ...
  const auto second = std::span<const std::byte>{encoded.value().data() + 7,
                                                encoded.value().size() - 7};
