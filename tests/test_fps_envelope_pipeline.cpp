...
auto material(std::uint8_t seed) -> fps::AeadMaterial {
  ...
  for (std::size_t i = 0; i < out.nonce_salt.size(); ++i) {
    out.nonce_salt[i] = static_cast<std::byte>(seed + 40U + static_cast<std::uint8_t>(i));
  }
  return out;
}
...
BOOST_AUTO_TEST_CASE(roundtrip_tls_record_preserves_inner_tls_and_frames) {
  fps::FpsEnvelopePipeline sender{
      fps::FpsEnvelopeCodec{config(fps::Direction::client_to_server)}};
  fps::FpsEnvelopePipeline receiver{
      fps::FpsEnvelopeCodec{config(fps::Direction::server_to_client)}};
  const auto inner_tls = bytes({0x17, 0x03, 0x03, 0x00, 0x04, 0xaa, 0xbb, 0xcc, 0xdd});
  const auto payload = bytes({0x45, 0x00, 0x00, 0x2c});

  auto wire = sender.encode_tls_record(fps::FpsEnvelopeContent{
...
BOOST_AUTO_TEST_CASE(buffers_fragmented_tls_record_until_boundary) {
  ...
  auto first = receiver.process_inbound_tls(
      std::span<const std::byte>{wire.value().data(), 7U});
  ...
  auto second = receiver.process_inbound_tls(
      std::span<const std::byte>{wire.value().data() + 7,
                                 wire.value().size() - 7U});
...
BOOST_AUTO_TEST_CASE(trial_fallback_forwards_cover_until_first_valid_envelope) {
  ...
  auto envelope = sender.encode_tls_record(fps::FpsEnvelopeContent{
      .inner_tls_bytes = inner_tls,
      .frames = {},
      .padding_size = 0,
  });
...
BOOST_AUTO_TEST_CASE(encode_surfaces_envelope_and_record_errors) {
  ...
  auto envelope_error = envelope_limited.encode_tls_record(fps::FpsEnvelopeContent{
      .inner_tls_bytes = bytes({1, 2}),
      .frames = {},
      .padding_size = 0,
  });
  BOOST_REQUIRE(!envelope_error);
  BOOST_CHECK(envelope_error.error().stage ==
              fps::FpsEnvelopePipelineEncodeStage::envelope);
  BOOST_CHECK(envelope_error.error().envelope_error ==
              fps::FpsEnvelopeError::oversized_inner_tls);
  ...
  auto record_error = record_limited.encode_tls_record(fps::FpsEnvelopeContent{
      .inner_tls_bytes = {},
      .frames = {},
      .padding_size = 0,
  });
  BOOST_REQUIRE(!record_error);
  BOOST_CHECK(record_error.error().stage ==
              fps::FpsEnvelopePipelineEncodeStage::tls_record);
  BOOST_CHECK(record_error.error().tls_record_error ==
              fps::TlsRecordLayerError::payload_too_large);
}
