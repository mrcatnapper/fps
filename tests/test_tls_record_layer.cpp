...
BOOST_AUTO_TEST_CASE(forwards_non_application_data_without_classifying) {
    ...
    const auto filtered = fps::filter_tls_records(std::span<const fps::TlsRecord>{&handshake, 1}, [&](const fps::TlsRecord&) {
...
BOOST_AUTO_TEST_CASE(strips_classified_application_data_and_extracts_payload) {
    ...
    const auto filtered = fps::filter_tls_records(records, [](const fps::TlsRecord& record) {
...
BOOST_AUTO_TEST_CASE(empty_classifier_forwards_application_data) {
    ...
    const auto filtered = fps::filter_tls_records(std::span<const fps::TlsRecord>{&real, 1}, fps::CovertRecordClassifier{});
...
BOOST_AUTO_TEST_CASE(filters_records_from_coalesced_parser_output) {
    ...
    const auto filtered = fps::filter_tls_records(parsed.records, [](const fps::TlsRecord& record) {
        const auto payload = record.payload();
...
BOOST_AUTO_TEST_CASE(reports_malformed_record_without_forwarding) {
    ...
    const auto filtered = fps::filter_tls_records(std::span<const fps::TlsRecord>{&malformed, 1}, [](const fps::TlsRecord&) { return false; });
